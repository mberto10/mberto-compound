...
// result_limit = Max rows in ranked risk tables (default: compact=25, full=100, min: 1, max: 300)
// headline_limit_per_symbol = Max headlines kept per symbol (default: 5, min: 1, max: 20)
// event_limit_per_symbol = Max events kept per symbol (default: 6, min: 1, max: 30)
...
const INPUT_ALIASES = {
  ...
  headline_limit_per_symbol: ['headlineLimitPerSymbol'],
  event_limit_per_symbol: ['eventLimitPerSymbol'],
};
...
const resultLimit = clampNumber(getCanonicalInput(input, 'result_limit', INPUT_ALIASES.result_limit, outputMode === 'compact' ? 25 : 100, inputCompatibility), outputMode === 'compact' ? 25 : 100, 1, 300);
const headlineLimitPerSymbol = clampNumber(getCanonicalInput(input, 'headline_limit_per_symbol', INPUT_ALIASES.headline_limit_per_symbol, 5, inputCompatibility), 5, 1, 20);
const eventLimitPerSymbol = clampNumber(getCanonicalInput(input, 'event_limit_per_symbol', INPUT_ALIASES.event_limit_per_symbol, 6, inputCompatibility), 6, 1, 30);
