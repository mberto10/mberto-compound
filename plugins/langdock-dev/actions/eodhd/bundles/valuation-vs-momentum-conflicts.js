...
try {
  ...
  const bullishConvergence = rows.filter((r) => r.setupType === 'bullish-convergence').slice(0, resultLimit);
  const bearishConvergence = rows.filter((r) => r.setupType === 'bearish-convergence').slice(0, resultLimit);
  const conflictExpensiveStrong = rows.filter((r) => r.setupType === 'conflict-expensive-strong').slice(0, resultLimit);
  const conflictCheapWeak = rows.filter((r) => r.setupType === 'conflict-cheap-weak').slice(0, resultLimit);
  const neutralOrOther = rows.filter((r) => r.setupType === 'unclassified').slice(0, resultLimit);
